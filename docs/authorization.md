...
| **Recruiter** | `job:create`, `job:update`, `job:publish`, `job:close`, `job:read`, `application:read`, `application:screen`, `application:shortlist`, `pipeline:manage`, `interview:schedule`, `interview:read`, `evaluation:read`, `talentPool:manage`, `analytics:read` |
| **Hiring Manager** | `application:read`, `evaluation:read`, `application:decide` (hire/reject), `job:read`, `analytics:read` |
